...
public class Box implements Iterable<Ball> {
    ...
    /**
     * maxVolume is used to make the max volume accessible to all the methods
     */
    ...
    public boolean add(Ball b) {
    	double temp = b.getVolume() + ballContainer.getVolume();
    	if (!ballContainer.contains(b) && (temp <= maxVolume)) {
    		ballContainer.add(b);
    		return true;    		
    	}
    	return false;
    ...
    public Iterator<Ball> getBallsFromSmallest() {
    	...
    	for (Ball b : ballContainer) {
    		int i = 0;
    		if(!list.isEmpty()) {
    			while(i < list.size() && (b.getVolume() >= list.get(i).getVolume())) {
    				i++;
    			}   			
    		}
    		list.add(i, b);
