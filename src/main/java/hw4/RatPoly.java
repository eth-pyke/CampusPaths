...
public final class RatPoly {
  ...
  public int degree() {
	  ...
	  // {{Inv: For each RatTerm in terms, its exponent <= maxDegree}}
	  for (RatTerm term : terms) {
		  if (term.getExpt() > maxDegree) {
			  maxDegree = term.getExpt();
		  }
	  }
  ...
  public RatTerm getTerm(int deg) {
	  // {{Inv: For each RatTerm in terms so far, its exponent !=j deg}}
	  for (RatTerm term : terms) {
		  if (term.getExpt() == deg) {
			  return term;
		  }
  ...
  public boolean isNaN() {
	  // {{Inv: For each term in terms so far, its coefficient != NaN}}
	  for (RatTerm term : terms) {
  ...
  private static void sortedInsert(List<RatTerm> lst, RatTerm newTerm) {
	  if (!newTerm.equals(RatTerm.ZERO)) {
		  ...
		  // {{Inv: i < lst.size() && lst's largest exponent is greater than the new terms 
		  //																		exponent}}
		  while (i < lst.size() && lst.get(i).getExpt() > newTerm.getExpt()) {
			  i++;
		  }
		  
		  // If lst is empty or newTerm's exponent does not exist in lst
		  if (i == lst.size()) {
			  lst.add(newTerm);
		  // If a term exists with same exponent
		  } else if (lst.get(i).getExpt() == newTerm.getExpt()) {
			  lst.set(i, lst.get(i).add(newTerm));
			  
			  // Remove if the addition was result was a 0 term
  ...
  public RatPoly negate() {
	  ...
	  // {{Inv: For each term in terms so far, the sign is changed}}
	  for (RatTerm term : terms) {
  ...
  public RatPoly add(RatPoly p) {
	  ...
	  // Makes a term for term copy of terms
	  for (RatTerm term : terms) {
		  list.add(term);
	  }
	  ...
	  // {{Inv: i >= 0 and list is equal to itself plus p's terms >= i}}
	  while (i >= 0) {
		  if (!p.getTerm(i).equals(RatTerm.ZERO)) {
  ...
  public RatPoly sub(RatPoly p) {
	  ...
	  // Returns a negated p added to this
  ...
  public RatPoly mul(RatPoly p) {
	  ...
	  // {{Inv: temp = p * each term in terms so far}}
	  for (RatTerm term : terms) {
		  
		  // {{Inv: temp = temp + term * p with degree i}}
		  for (int i = p.degree(); i >= 0; i--) {
			  if (!p.getTerm(i).equals(RatTerm.ZERO)) {
				  sortedInsert(temp, term.mul(p.getTerm(i)));
			  }
		  }
	  }
  ...
  public RatPoly div(RatPoly p) {
	  ...
	  // {{Inv: r != ZERO and p's max degree is less than r's max degree}}
	  // Locates gcf between each RatPoly's biggest term then adds that to temp
	  while (!r.equals(ZERO) && p.degree() < r.degree()) {
		  RatNum coeff = r.getTerm(r.degree()).getCoeff().div(p.getTerm(p.degree()).getCoeff());
		  RatTerm term = new RatTerm(coeff, r.degree() - p.degree());
		  
		  temp = temp.add(new RatPoly(term));
		  r = r.sub(p.mul(new RatPoly(term)));
	  }
	  
	  // Test for when each largest degree is equal
	  if (!r.equals(ZERO) && p.degree() == r.degree()) {
		  RatNum coeff = r.getTerm(r.degree()).getCoeff().div(p.getTerm(p.degree()).getCoeff());
		  RatTerm term = new RatTerm(coeff, 0);
  ...
  public RatPoly differentiate() {
	  ...
	  // {{Inv: temp = the sum of each derivative of each term in terms so far}}
	  for (RatTerm term : terms) {
		  if (!term.differentiate().equals(RatTerm.ZERO)) {
			  temp.add(term.differentiate());
		  }
	  }
  ...
  public RatPoly antiDifferentiate(RatNum integrationConstant) {
	  ...
	  // {{Inv: temp = the sum of each antiderivative of each term in terms so far}}
	  for (RatTerm term : terms) {
		  temp.add(term.antiDifferentiate());
	  }
  ...
  public double eval(double d) {
	  ...
	  // {{Inv: temp = each term in terms evaluated at d so far}}
	  for (RatTerm term : terms) {
		  temp = temp + term.eval(d);
	  }
